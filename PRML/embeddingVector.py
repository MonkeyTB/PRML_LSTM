...
def embeddingNoteVector(FilePath,DiDr,r,d):
	...
	for i in range(Pst_array.shape[0]):
		mid_list = []
		drugNum = Pst_array[i][1]
		midNum = Pst_array[i][2]
		disNum = Pst_array[i][3]
		# if Pst_array[i][0] == 0 or Pst_array[i][0] == 1:
		if drugNum == r and  disNum == d:
			mid_list.append([drugNum, disNum])  # 起始药物ID和终止疾病ID
			flag = True
		if flag == True:
			mid_list.append(DiDr[ drugNum ])  # 第一种药物
			if  Pst_array[i][0] == 0:		# r-r-d
				mid_list.append( DiDr[ midNum ])	#第二种药物
				mid_list.append( DiDr[ disNum + 763 ])	#第一种疾病
				mid_list.append( [0]*DiDr.shape[0] )					#最后元素补0
				embeddingVector_list.append(mid_list)
			if  Pst_array[i][0] == 1:		# r-d-d
				mid_list.append(DiDr[ midNum + 763 ])	#第一种疾病
				mid_list.append(DiDr[ disNum + 763 ])	#第二种疾病
				mid_list.append([0] * DiDr.shape[0] )  # 最后元素补0
				embeddingVector_list.append(mid_list)
